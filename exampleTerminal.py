...
lapEventType = wx.NewEventType()
EVT_LAP_EVENT = wx.PyEventBinder(lapEventType, 1)
...
class LapEvent(wx.PyCommandEvent):
...
class TerminalFrame(wx.Frame):
    ...
    def __init__(self, *args, **kwds):
        ...
        self.serial.timeout = 0.2   #make sure that the alive event can be checked from time to time
        ...
        self.host = "http://localhost:8000/api/v1/lap/?format=json"
        self.remote_link = wx.StaticText(self, -1, self.host)
        ...
        wxglade_tmp_menu.Append(ID_CLEAR, "Wy&czysc", "", wx.ITEM_NORMAL)
        wxglade_tmp_menu.Append(ID_SAVEAS, "&Zapisz tekst jako...", "", wx.ITEM_NORMAL)
        wxglade_tmp_menu.AppendSeparator()
        wxglade_tmp_menu.Append(ID_SETTINGS, "&Ustawienia portu...", "", wx.ITEM_NORMAL)
        wxglade_tmp_menu.Append(ID_TERM, "Ustawienia &terminala...", "", wx.ITEM_NORMAL)
        wxglade_tmp_menu.AppendSeparator()
        wxglade_tmp_menu.Append(ID_EXIT, "&Wyjdz", "", wx.ITEM_NORMAL)
        self.frame_terminal_menubar.Append(wxglade_tmp_menu, "&Plik")
        # Menu Bar end
    ...
    def __do_layout(self):
        ...
        sizer_4 = wx.StaticBoxSizer(wx.StaticBox(self, -1, "Race Record Web:"), wx.HORIZONTAL)
        sizer_4.Add(self.remote_link, 1, wx.EXPAND, 0)
    ...
    def __attach_events(self):
        ...
        self.Bind(EVT_LAP_EVENT, self.ColorSwitch)
    ...
    def OnSerialRead(self, event):
        ...
        self.text_ctrl_output.AppendText(u'Wystartowal {}'.format(str(text)))

    def ColorSwitch(self, evt):
        time = evt.GetLapTime()
        #print(color)
        self.text_ctrl_output.AppendText(u' uzyskal czas {}'.format(str(time)))
    ...
    def readBytes(self, num):
        ...
        while num > 0:
            ...
            if x:
                result = result + str(self.decode(x)) + ' '
            num = num -1
        return result
    ...
    def ComPortThread(self):
        ...
        while self.alive.isSet():               #loop while alive event is true
            ...
            print 'Odebralem znak {}'.format(text)
            ...
            #        text = text + self.serial.read(n) #get it
                    ...
                    stnr = self.decode(self.serial.read(1))
                    print stnr
                    event = SerialRxEvent(self.GetId(), stnr)
                    self.GetEventHandler().AddPendingEvent(event)
                    
                if text == 0:
                    time = self.readBytes(3)
                    print time
        
                    lapevt = LapEvent(lapEventType, -1)     
                    lapevt.SetLapTime(time)
                    self.GetEventHandler().ProcessEvent(lapevt)

                    #event = LapEvent(self.GetId(), time)
                    #self.GetEventHandler().AddPendingEvent(event)
